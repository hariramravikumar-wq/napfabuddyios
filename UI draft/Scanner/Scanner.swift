...
struct Scanner: View {
    ...
    private func persistScoreForStudent(studentID: String, studentName: String, station: String, rawScore: String) {
        let unit: String
        switch station {
        case "Shuttle Run", "1.6km Run": unit = "sec"
        case "Standing Broad Jump": unit = "cm"
        default: unit = "reps"
        }
        let formattedScore = "\(rawScore) \(unit)"
        let db = Firestore.firestore()
        // Save to scores subcollection (per-station document)
        db.collection("students")
            .document(studentID)
            .collection("scores")
            .document(station)
            .setData([
                ...
                if let error = error {
                    ...
                    print("Score saved for \(studentName) — \(station): \(formattedScore)")
                }
            }
        ...
        db.collection("students")
            ...
            .addDocument(data: [
                ...
                if let error = error {
                    print("Failed to append activity log: \(error.localizedDescription)")
                }
            }
    }
                ...
                VStack(spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Button {
                            ...
                            if let sid = students.first(where: { $0.name == trimmedName })?.id {
                                persistScoreForStudent(studentID: sid, studentName: trimmedName, station: station, rawScore: quickAddScore)
                            } else {
                                print("Warning: Could not resolve student ID for \(trimmedName). Not saving to Firestore.")
                            }
                                            ...
                                            VStack(alignment: .leading, spacing: 8) {
                                                ...
                                                VStack(alignment: .leading, spacing: 12) {
                                                    ...
                                                    Button {
                                                        ...
                                                        guard let sid = students.first(where: { $0.name == participant })?.id else {
                                                            print("Warning: Could not resolve student ID for \(participant). Not saving to Firestore.")
                                                            return
                                                        }
                                                        for s in stations {
                                                            guard let raw = allStationScores[s]?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty else { continue }
                                                            // Update local best record list
                                                            let unit: String
                                                            switch s {
                                                            case "Shuttle Run", "1.6km Run": unit = "sec"
                                                            case "Standing Broad Jump": unit = "cm"
                                                            ...
                                                            persistScoreForStudent(studentID: sid, studentName: participant, station: s, rawScore: raw)
                                                        }
                                                    } label: {
                                                        HStack(spacing: 8) {
